...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 27 16:04:02 2021
...
puzzles = pd.DataFrame(columns = ['puzzle','solution','difficulty'])
...
while runs < leng:
    ...
    for row in sudoku.solution:
        sol.extend(row)
    solution = ''.join([str(x) for x in sol])
        
    for row in sudoku.grid:
        puz.extend(row)
    puzzle = ''.join([str(x) for x in puz])
    
    base_url = "https://www.thonky.com/sudoku/evaluate-sudoku?puzzlebox="
    url = base_url + str(puzzle)
    ...
    difficulty = [int(i) for i in diffstr.split() if i.isdigit()][0]
    
    tempgl = len(puzzles[puzzles['difficulty'] == difficulty])
    
    if difficulty <= 2 and tempgl > lowlim:
        flg = 0
    else:
        flg = 1
    ...
    if flg == 1:
        row = [puzzle, solution, difficulty]
        puzzles.loc[gl] = row
        print(str(runs) + ': ' + str(difficulty))
    

print("Finished " + str(leng) + " puzzles in " + str(datetime.now() - start))
print(puzzles.groupby(['difficulty']).count())